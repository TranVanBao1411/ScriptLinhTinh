...
local Weapon = nil
local tab1 = Win:CraftTab('Function')
local tab2 = Win:CraftTab('More')
local tab3 = Win:CraftTab('Tele')
...
page1:Toggle('Auto Farm Test',false,function(value)
Farm = value
end)

page1:Toggle('Fast Attack (Lag)',false,function(value)
    CBLRaid = value
end)

local page2 = tab1:CraftPage('SHIT',2)

page2:Label('Config')

page2:Dropdown("Farm Method",{"Normal","Fast"},"Normal",function(a)
    Method = a
end)
...
page2:Dropdown("Item",Weaponlist,"Combat",function(a)
    Weapon = a
end)


local page1_2 = tab2:CraftPage('Player',1)

page1_2:Label('Mics')

page1_2:Toggle('Auto Esp Fruit',false,function(va)
    DevilFruitESP = va
...
page1_2:Button('Teleport to Raid',function()
...
local page2_2 = tab2:CraftPage('Player',2)
...
local page1_3 = tab3:CraftPage('TeleIsland',1)

page1_3:Dropdown("Island",{"FirstIsland","cc"},nil,function(a)
    if a == "FirstIsland" then
        print("cc")
    elseif a == "cc" then
        print("đụ má m")
    end
					...
					for i,x in pairs(game:GetService("Workspace").Npc:GetChildren()) do
						if x.Name == NameMon then
							if x:FindFirstChild("Humanoid") and x:FindFirstChild("Humanoid").Health ~= 0 and x:FindFirstChild("HumanoidRootPart") then
								...
								until x:FindFirstChild("Humanoid").Health <= 0 or not Farm or  game:GetService("Players").LocalPlayer.Quest.Num.Value == 0
							end
						end
					end
...
local Event = game:GetService("Players")["0om4"].PlayerGui.QuestBar.Background.Cancle.RemoteEvent
Event:FireServer()
                    end
				end
			end)
...
if game:GetService("Players").LocalPlayer.Character[Vtool]:FindFirstChild("DungCoHackNua") then
...
game.Players.LocalPlayer.Character.ChildAdded:Connect(function(tools)
    if tools:IsA("Tool") then
        if tools:FindFirstChild("DungCoHackNua") or tools:FindFirstChild("XaiLaBiBanAcc") then 
            Vtool = tools.Name
            print(tools.Name)
        end
    end
